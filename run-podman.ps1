# Run Podman Containers Script for Windows PowerShell
# Starts backend and frontend containers

Write-Host "🚀 Starting Podman containers..." -ForegroundColor Cyan
Write-Host ""
...
$backendRunning = podman ps --filter "name=readlife-backend" --format "{{.Names}}" 2>$null
$frontendRunning = podman ps --filter "name=readlife-frontend" --format "{{.Names}}" 2>$null

# Stop and remove existing containers if they exist
if ($backendRunning) {
    Write-Host "🛑 Stopping existing backend container..." -ForegroundColor Yellow
    podman stop readlife-backend 2>$null
    podman rm readlife-backend 2>$null
}

if ($frontendRunning) {
    Write-Host "🛑 Stopping existing frontend container..." -ForegroundColor Yellow
    podman stop readlife-frontend 2>$null
    podman rm readlife-frontend 2>$null
}

# Check if images exist
$backendImage = podman images --format "{{.Repository}}:{{.Tag}}" | Select-String "readlife-backend:latest"
$frontendImage = podman images --format "{{.Repository}}:{{.Tag}}" | Select-String "readlife-frontend:latest"

if (-not $backendImage) {
    Write-Host "❌ Backend image not found. Please run .\build-podman.ps1 first" -ForegroundColor Red
    exit 1
}

if (-not $frontendImage) {
    Write-Host "❌ Frontend image not found. Please run .\build-podman.ps1 first" -ForegroundColor Red
    exit 1
}
...
$networkExists = podman network ls --format "{{.Name}}" | Select-String "readlife-network"
if (-not $networkExists) {
    Write-Host "🌐 Creating Podman network..." -ForegroundColor Yellow
...
Write-Host "   Remove:       podman rm readlife-backend readlife-frontend" -ForegroundColor Gray
